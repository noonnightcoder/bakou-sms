...
class Promotions_model extends CI_Model {
    ...
    */
    public function get_all($search_string=null, $limit_start=null, $limit_end=null)
    {
        ...
        if($search_string){
            $this->db->like('lower(promotion_name)', strtolower($search_string));
        }
        $this->db->group_by('id');
    ...
    public function get_promotion_by_type($type)
    {
        $this->db->select('*');
        $this->db->from('promotions');
        $this->db->where('status', 1);
        $this->db->where('effective_end >= NOW()');
        $this->db->like('lower(promotion_type)', strtolower($type));
